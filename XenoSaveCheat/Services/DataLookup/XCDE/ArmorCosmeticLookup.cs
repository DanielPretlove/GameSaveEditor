/*
 * XenoSaveCheatCore
 * Copyright (C) 2023  damysteryman
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */

using dmm.XenoSave.XCDE;
namespace XenoSaveCheat.Services.DataLookup.XCDE;

public class ArmorCosmeticLookup : GenericDataLookup, IDataLookupObject
{
    public bool Head  { get; set; }
    public bool Torso { get; set; }
    public bool Arm   { get; set; }
    public bool Leg   { get; set; }
    public bool Foot  { get; set; }

    public bool IsHead()  => Head;
    public bool IsTorso() => Torso;
    public bool IsArm()   => Arm;
    public bool IsLeg()   => Leg;
    public bool IsFoot()  => Foot;
}